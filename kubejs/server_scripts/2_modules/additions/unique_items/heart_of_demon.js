const demon_sfx = {
    "entity.warden.heartbeat": { volume: 2, pitch: 0.6},
    "entity.warden.roar": { volume: 1, pitch: 1.45},
    "block.beacon.deactivate": { volume: 1, pitch: 1},
    "entity.wither.spawn": { volume: 1.36, pitch: 1.9},
    "entity.wither.hurt": { volume: 0.5, pitch: 0.6},
    "entity.warden.heartbeat": { volume: 2, pitch: 0.6},
    "entity.elder_guardian.ambient": { volume: 0.6, pitch: 0.6},
    "item.totem.use": { volume: 0.5, pitch: 0.8},
    "entity.firework_rocket.large_blast_far": { volume: 1, pitch: 0.6}
};
const demon_effects = {
    "regeneration": { time: 30, amp: 5 },
    "fire_resistance": { time: 800 },
    "kubejs:invincible": { time: 60 },
    "wither": { time: 30, particle: false },
    "saturation": { time: 30, amp: 2, particle: false }
};

function heart_of_demon(e) {
    const { level, server, entity } = e;
    const { x, y, z } = entity;

    if (!entity.isHoldingInAnyHand("kubejs:heart_of_demon")) return;
    if (player.maxHealth <= 6) return;

    global.totemAnimation(entity, "kubejs:heart_of_demon");
    global.particleBurst(level, entity, "soul_fire_flame", 30, 0.4);
    level.spawnParticles("alessandrvenchantments:enderwave", true, x, y, z, 0, 0, 0, 1, 0);

    Object.keys(demon_sfx).forEach(s => {
        const sfxInfo = demon_sfx[s];
        global.sound(level, entity, s, sfxInfo.volume, sfxInfo.pitch)
    });
    server.scheduleInTicks(30, () => global.sound(level, entity, "entity.warden.heartbeat", 2, 0.6));

    level.getEntitiesWithin(entity.boundingBox.inflate(4, 2, 4)).forEach((entity2) => {
        if (!entity2.isLiving() || entity2 == entity) return;
        entity2.knockback(1.0, x - entity2.x, z - entity2.z);
    });

    global.updateMaxHealth(player, -2);

    effectPack(demon_effects, entity);
    entity.setHealth(2.0);
    e.cancel();
}